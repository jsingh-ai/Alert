// SQLite-only development seed.
// Remove this file with prisma/sqlite/schema.prisma when PostgreSQL is the only target.
...
type ScopeTypeValue = (typeof ScopeType)[keyof typeof ScopeType];
...
async function user(username: string, displayName: string, role: RoleValue, companyId: string, password = `${username}123`) {
...
async function resetScopes(membershipId: string, scopes: Array<{ scopeType: ScopeTypeValue; scopeId: string }>) {
  await prisma.membershipScope.deleteMany({ where: { membershipId } });
  if (scopes.length) {
    await prisma.membershipScope.createMany({ data: scopes.map((scope) => ({ ...scope, membershipId })) });
  }
}

async function main() {
  await prisma.$executeRawUnsafe(
    "CREATE UNIQUE INDEX IF NOT EXISTS andon_alert_active_machine_department_idx ON andon_alerts (machine_id, department_id) WHERE status IN ('OPEN', 'ACKNOWLEDGED', 'ARRIVED')"
  );
  ...
  const pressGroup = await prisma.machineGroup.upsert({
  ...
  const packagingGroup = await prisma.machineGroup.upsert({
  ...
  const issues: Record<string, Awaited<ReturnType<typeof prisma.issueType.upsert>>> = {};
  ...
  async function template(name: string, buttonLabel: string, color: string, sortOrder: number, targets: Array<{ department: string; issue: string; message?: string | null; priority?: PriorityValue }>) {
  ...
  console.log("Demo users: admin/admin123, manager/manager123, operator/operator123, quality/quality123, supervisor/supervisor123, viewer/viewer123");
}
